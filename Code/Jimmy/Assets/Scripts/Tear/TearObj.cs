...
public class TearObj : MonoBehaviour, IClickable
{
	...
	private void Start()
	{
		GameManager1_1.Instance.SetAway.Add(meshObj.AddComponent<ExitFromScene>());
		GameManager1_1.Instance.SetAway.Add(meshObj2.AddComponent<ExitFromScene>());
		GameManager1_1.Instance.SetAway.Add(meshObjBack.AddComponent<ExitFromScene>());
		GameManager1_1.Instance.SetAway.Add(meshObjBack2.AddComponent<ExitFromScene>());
	}
	private void Update()
	{
		if (startDrag)
	...
	private void TearInit(float time, Vector2 start, Vector2 end)
	{
		...
		deltaVec = (end - start) / time * Time.fixedDeltaTime;
		timer = 0;
		TargetPos = start;
	...
	IEnumerator IChangeTarPos(float duration)
	{
		...
		while (iChangeTarTimer < duration)
		{
			...
			if (!rotated)
			{
				...
				Debug.LogWarning("Successfully created");
			}
			else
			{
...
public class Compare : IComparer<Vector3>
{
	Vector3 firstPoint;
	public void DeliverFirstPoint(List<Vector3> vectors)
    {
		...
		for (int i = 0; i < vectors.Count; i++)
        {
			...
			else if (vectors[i].x == minX)
            {
				minY = Mathf.Min(vectors[i].y, minY);
            }
        }
		firstPoint = new Vector2(minX, minY);
    }
    int IComparer<Vector3>.Compare(Vector3 x, Vector3 y)
    {
		if (x == firstPoint)
			return -1;
		else if (y == firstPoint)
			return 1;

		Vector2 v1 = new Vector2(1, 0);
		Vector2 v2 = x - firstPoint;
		float multi;
		if (x.y > firstPoint.y)
			multi = 1;
		else
			multi = -1;
		float angleA = Mathf.Acos(Vector2.Dot(v1, v2) / (v1.magnitude * v2.magnitude)) * multi;
		Vector2 v3 = y - firstPoint;

		if (y.y > firstPoint.y)
			multi = 1;
		else
			multi = -1;
		float angleB = Mathf.Acos(Vector2.Dot(v1, v3) / (v1.magnitude * v3.magnitude)) * multi;

		if (Mathf.Tan(Mathf.Deg2Rad * angleA) > Mathf.Tan(Mathf.Deg2Rad * angleB))
			return -1;
		else if (Mathf.Tan(Mathf.Deg2Rad * angleA) < Mathf.Tan(Mathf.Deg2Rad * angleB))
			return 1;
		return 0;
    }
}
public class line
{
	...
	public line(Vector2 startPos, Vector2 endPos)
    {
		...
		this.endPos = endPos;
		if (Mathf.Abs(startPos.x - endPos.x) > 0.05f)
        {
			k = (float)(endPos.y - startPos.y) / (float)(endPos.x - startPos.x);
		}
        else
        {
			Vertical = true;
	...
	public line(Vector2 pos, float kk)
    {
		k = kk;
	...
	public Vector2 CrossPoint(line line1)
    {
		float posX = 0, posY = 0;
		bool inside;
		
		if (!line1.Vertical && !Vertical)
        {
			posY = startPos.y;
			posX = line1.GetX(posY);

		}
		else if (!line1.Vertical && Vertical)
        {
			posX = x;
			posY = line1.GetY(x);
		...
		if ((posX - startPos.x) * (endPos.x - posX) >= 0f && (posY - startPos.y) * (endPos.y - posY) >= 0f)
			inside = true;
		else
			inside = false;
		Inside = inside;
